...
for port in $DIRECTTCP; do
  ...
  iptables -t nat -A POSTROUTING -o eth0 -p tcp --dport \$port -j SNAT --to-source $EXT_IP_V4
done
...
for port in $DIRECTTCP; do
  ...
  iptables -t nat -D POSTROUTING -o eth0 -p tcp --dport \$port -j SNAT --to-source $EXT_IP_V4
done
